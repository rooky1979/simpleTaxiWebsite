<!--Matthew Rook ID: 18007407
	admin.php for server side to retrieve the bookings from the DB and tabulate-->
<?php
...
//query to update the unassigned field in the db to assigned
if ($updaterefno != "") {
	$query = "UPDATE `taxiBookings` SET status = 'assigned' WHERE reference_id = '$updaterefno'";
	$result = mysqli_query($conn, $query);  
	$query = "SELECT * FROM taxiBookings WHERE reference_id LIKE '$updaterefno'";
	$result = mysqli_query($conn, $query);
	$count = mysqli_num_rows($result);

//if no results found, display the error message
if ($count == 0) {
	echo "<p>Error building the table</p>";
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		echo "<tr><td>$refno</td>
				  ...
				  <td>$time</td>
				  <td>$status</td>
				  <td><input name=\"abutton\" type=\"button\" onClick=\"assignTaxi('admin.php', 'targetDiv', $refno)\" value=\"Assign\"></td></tr>";
	}
	echo "</table>";
}
}else if ($bsearch == "") {
...
//if no results found, display the error message
if ($count == 0) {
	echo "<p>There are no bookings for $currentdate between $currenttime and $plustwohours</p>";
	//else create the table
} else {
	echo "<table>";
	//table headings
...
if ($count == 0) {
	...
	echo "<p>Date: $currentdate Time: $currenttime Time: $plustwohours</p>";
	//else create the table
} else {
	echo "<table>";
	//table headings
